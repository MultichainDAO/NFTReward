...
contract RewardHandler_VEShare is Administrable, IRewardHandler, IERC721Receiver {
    address public ve;
    ...
    function withdrawReward(uint256 amount) onlyAdmin external {
        IERC20(IReward(vereward).rewardToken()).transfer(msg.sender, amount);
        emit LogWithdrawReward(amount);
    }

    function withdrawVe(uint256 tokenId) onlyAdmin external {
        IVE(ve).safeTransferFrom(address(this), msg.sender, tokenId);
        emit LogWithdrawVE(ve_tokenId);
        ve_tokenId = 0;
    }

    function withdrawVe() onlyAdmin external {
        IVE(ve).safeTransferFrom(address(this), msg.sender, ve_tokenId);
        emit LogWithdrawVE(ve_tokenId);
        ve_tokenId = 0;
    }
    ...
    function claimable(uint256 tokenId) override external view returns(uint256) {
